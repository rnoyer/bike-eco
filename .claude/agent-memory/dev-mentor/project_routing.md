...
- Per-screen options (e.g. hiding the header) are set by rendering `<Stack.Screen options={{...}} />` _inside_ the screen component, not only in `_layout.tsx`. The root layout is just `<Stack />` with default `headerShown`. See `src/app/b2cSubmissionForm.tsx` and `src/app/index.tsx`.
